...
# it can be flashed via the /dev/btt-manta-m4p path, but it then fails
...
cp -f /home/pi/klipper_config/config/boards/btt-manta-m4p/firmware.config /home/pi/klipper/.config
...
cp -f /home/pi/klipper/out/klipper.bin /home/pi/klipper_config/firmware_binaries/firmware-btt-manta-m4p.bin
chown pi:pi /home/pi/klipper_config/firmware_binaries/firmware-btt-manta-m4p.bin

# Reset ownership
chown pi:pi -R /home/pi/klipper

# echo "Unfortunately, unlike the Manta M4P cannot currently be flashed via DFU. The file firmware-btt-manta-m4p.bin has been compiled and is available in the firmware_binaries folder in Mainsail under the Machine tab. Use this to flash via SD Card." 
...
if [ -h $MCU ]; then
    ...
    echo "Flashing Manta M4P via vendor and device ids - 1st pass"
...
if [ -h $MCU ]; then
    ...
    echo "Flashing Manta M4P via vendor and device ids - 2nd pass"
    make flash FLASH_DEVICE=$VENDORDEVICEID
    ...
    if [ -h $MCU ]; then
        ...
        echo "Flashing Manta M4P via vendor and device ids - 3rd pass"
        make flash FLASH_DEVICE=$VENDORDEVICEID
        if [ $? -e 0 ]; then
            echo "Flashing successful!"
        else
