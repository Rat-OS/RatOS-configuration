...
class RMMUCommandHelper:
	...
	def register_commands(self, name):
		...
		gcode.register_mux_command('RMMU_SET_FILAMENT_SENSOR', 'RMMU', name, self.cmd_RMMU_SET_FILAMENT_SENSOR, desc=(self.desc_RMMU_SET_FILAMENT_SENSOR))
	...
	def cmd_RMMU_SET_FILAMENT_SENSOR(self, param):
		self.rmmu.set_filament_sensor(param)
...
class RMMU:
	...
	def load_settings(self):
		...
		# feeder pins
		if self.config.get('feeder_t0_pin', None) is not None:
			# Tx feeder pins
			for i in range(0, self.tool_count):
	...
	def get_status(self, eventtime):
		return {'name': self.name,
		  ...
		  'toolhead_filament_sensor': self.toolhead_filament_sensor.get_status(eventtime),}
	...
	def reset(self):
		...
		status_color_error = self.get_macro_variable("RatOS", "status_color_error")
		status_color_unknown = self.get_macro_variable("RatOS", "status_color_unknown")
		...
		if not self.dual_carriage:
			...
			for i in range(0, self.tool_count):
				if i == loaded_filament:
					...
					self.gcode.run_script_from_command('SET_GCODE_VARIABLE MACRO=T' + str(i) + ' VARIABLE=color VALUE=\'"' + status_color_ok + "\"\'")
				else:
					self.gcode.run_script_from_command("SET_GCODE_VARIABLE MACRO=T" + str(i) + " VARIABLE=active VALUE=False")
					self.gcode.run_script_from_command('SET_GCODE_VARIABLE MACRO=T' + str(i) + ' VARIABLE=color VALUE=\'"' + status_color_unknown + "\"\'")
		...
		for i in range(0, self.tool_count):
			if tool == -1 or tool == i:
				...
				if not self.home_filament(i):
					# update frontend
					if not self.dual_carriage:
						self.gcode.run_script_from_command('SET_GCODE_VARIABLE MACRO=T' + str(i) + ' VARIABLE=color VALUE=\'"' + status_color_error + "\"\'")
					# console output
					self.console_echo({
						'TITLE': "Homing filament", 
				...
				if self.parking_sensor_endstop != None:
					if self.is_endstop_triggered(self.parking_sensor_endstop):
						...
						self.console_echo({
							'TITLE': "Homing filament", 
							'MSG': 	"Parking filament sensor isssue detected!\nFilament homing stopped.", 
							'TYPE': "alert"
						})
						# pull filament back
				...
				# check toolhead sensor
					...
					if self.toolhead_filament_sensor.filament_present:
						...
						self.console_echo({
							'TITLE': "Homing filament", 
							'MSG': 	"Toolhead filament sensor isssue detected!\nFilament homing stopped.", 
							'TYPE': "alert"
						})
						# pull filament back
						self.select_filament(i)
	...
	def load_filament_from_parking_position_to_parking_sensor(self, tool):
		# echo
		self.ratos_debug_echo("Loading filament T" + str(tool) + " from parking position into parking sensor...")
		...
		if not self.is_endstop_triggered(self.parking_sensor_endstop):
			self.console_echo({
				'TITLE': "Load filament", 
				'MSG': 	"Could not load filament T" + str(tool) + " into parking sensor!", 
				'TYPE': "warning"
			})
			try_count = 5
			move_distance = 30
		...
		# echo
		self.ratos_debug_echo("Filament T" + str(tool) + " loaded into parking sensor!")
	...
	def load_filament_from_parking_sensor_to_toolhead_sensor(self, tool):
		# echo
		self.ratos_debug_echo("Loading filament T" + str(tool) + " from parking sensor to toolhead sensor...")
		...
		# echo
		self.ratos_debug_echo("Filament T" + str(tool) + " loaded into toolhead sensor!")
	...
	def load_filament_from_reverse_bowden_to_toolhead_sensor(self, tool):
		# echo
		self.ratos_debug_echo("Loading filament T" + str(tool) + " from reverse bowden into toolhead sensor...")
	...
	def extruder_test(self, tool):
		...
		for i in range(1, 5):
			if not self.push_and_pull_test(self.cooling_zone_loading_speed / i, self.cooling_zone_loading_accel / i):
				...
				# echo
				self.ratos_debug_echo("Extruder test successful!")
	...
	def push_and_pull_test(self, loading_speed, loading_accel):
		# echo
		self.ratos_debug_echo("Push and pull test...")
		...
		# return result
		return self.toolhead_filament_sensor.filament_present
	...
	def unload_filament_from_reverse_bowden_to_parking_sensor(self, tool):
		# echo
		self.ratos_debug_echo("Unloading filament T" + str(tool) + " from reverse bowden to parking sensor...")
		...
		if self.is_endstop_triggered(endstop):
			self.console_echo({
				'TITLE': "Unload filament", 
				'MSG': 	"Could not unload filament T" + str(tool) + " to parking sensor!", 
				'TYPE': "alert"
			})
			return False
	...
	def unload_filament_from_parking_sensor_to_parking_position(self, tool):
		...
		if self.is_endstop_triggered(self.parking_sensor_endstop):
			self.console_echo({
				'TITLE': "Unload filament", 
				'MSG': 	"Could not unload filament T" + str(tool) + " from parking sensor!", 
				'TYPE': "warning"
			})
			try_count = 5
			move_distance = 50
	...
	def unload_filament_from_cooling_zone_to_reverse_bowden(self, tool):
		# echo
		self.ratos_debug_echo("Unloading filament T" + str(tool) + " from cooling zone to reverse bowden...")
		...
		if self.toolhead_filament_sensor.filament_present:
			self.console_echo({
				'TITLE': "Unload filament", 
				'MSG': 	"Could not unload filament T" + str(tool) + " from cooling zone to reverse bowden!", 
				'TYPE': "warning"
			})
			if self.filament_cleaning_distance > 0:
				self.console_echo({
					'TITLE': "Load filament", 
					'MSG': 	"Trying to clean the toolhead filament sensor...", 
					'TYPE': "info"
				})
				...
				if not self.toolhead_filament_sensor.filament_present:
					self.console_echo({
						'TITLE': "Load filament", 
						'MSG': 	"Toolhead filament sensor successfully cleaned!", 
						'TYPE': "success"
					})
					return True
				self.console_echo({
					'TITLE': "Unload filament", 
					'MSG': 	"Could not clean toolhead filament sensor!", 
					'TYPE': "alert"
				})
			return False

		# echo
		self.ratos_debug_echo("Filament T" + str(tool) + " unloaded to reverse bowden!")
	...
	def unload_filament_from_toolhead_sensor_to_reverse_bowden(self, tool):
		# echo
		self.ratos_debug_echo("Unloading filament T" + str(tool) + " from toolhead sensor to reverse bowden...")
		...
		if self.toolhead_filament_sensor.filament_present:
			self.console_echo({
				'TITLE': "Unload filament", 
				'MSG': 	"Could not unload filament T" + str(tool) + " from toolhead sensor to reverse bowden!", 
				'TYPE': "alert"
			})
			return False
	...
	def test_filament(self, filament):
		if len(self.parking_t_sensor_endstop) == self.tool_count or len(self.feeder_sensor) == self.tool_count:
			if len(self.parking_t_sensor_endstop) == self.tool_count:
				if not self.is_endstop_triggered(self.parking_t_sensor_endstop[filament]):
					return False
			if len(self.feeder_sensor) == self.tool_count:
				if not self.feeder_sensor[filament].filament_present:
					return False
			return True
		else:
	...
	def eject_filament(self, tool):
		...
		if len(self.parking_t_sensor_endstop) == self.tool_count:
			...
			if self.is_endstop_triggered(self.parking_t_sensor_endstop[tool]):
				self.console_echo({
					'TITLE': "Eject filament", 
					'MSG': 	"Could not eject filament T" + str(tool) + "! Parking sensor still triggered.", 
					'TYPE': "alert"
				})
				return
	...
	def join_spools(self, param):
				...
				for joined_spool in spool_join:
					for new_spool in new_spools:
						if joined_spool == new_spool:
							self.console_echo({
								'TITLE': "Join Spools", 
								'MSG': 	"Spool T" + str(new_spool) + " already joined with another one!", 
								'TYPE': "alert"
							})
							self.echo_spool_join()
							return
		...
		if self.start_print_param != None:
			...
			if counter > 1:
				self.console_echo({
					'TITLE': "Join Spools", 
					'MSG': 	"Can not join spools because selected spools are part of the ongoing print!", 
					'TYPE': "alert"
				})
				return
			...
			if spool != tool:
				if self.test_filament(spool):
					if self.load_filament(spool):
						...
						self.console_echo({
							'TITLE': "Join spool", 
							'MSG': 	"Can not join spool T" + str(spool) + "!", 
							'TYPE': "warning"
						})
						self.select_filament(tool)
						...
						if self.toolhead_filament_sensor.filament_present:
							self.console_echo({
								'TITLE': "Join Spools", 
								'MSG': 	"Can not join spools! Toolhead sensor is still triggered after a unsuccessful filament loading.", 
								'TYPE': "alert"
							})
							return False
				else:
					self.console_echo({
						'TITLE': "Join spool", 
						'MSG': 	"Spool " + str(spool) + " not available!", 
						'TYPE': "warning"
					})
		return False
	...
	def remap_toolhead(self, param):
		...
		if self.start_print_param != None:
			self.console_echo({
				'TITLE': "Remap Toolhead", 
				'MSG': 	"Toolhead remapping is not supported during a print!", 
				'TYPE': "warning"
			})
			return
				...
				for toolhead in toolhead_map:
					for new_toolhead in new_toolhead_map:
						if toolhead == new_toolhead:
							self.console_echo({
								'TITLE': "Remap Toolhead", 
								'MSG': 	"Can not remap toolhead T" + str(new_toolhead) + "! Toolhead is already mapped.", 
								'TYPE': "alert"
							})
							self.echo_toolhead_mapping()
							return
	...
	def query_sensors(self):
		...
		if self.toolhead_filament_sensor != None:
			result += "Toolhead sensor triggered: " + str(self.toolhead_filament_sensor.filament_present) + "\n"
		if len(self.feeder_sensor) == self.tool_count:
			for i in range(0, self.tool_count):
				result += "Feeder sensor T" + str(i) + " triggered: " + str(self.feeder_sensor[i].filament_present) + "\n"
	...
	#####
	def set_filament_sensor(self, param):
		# parameter
		sensor = param.get('SENSOR', "").lower()
		enable = param.get_int('ENABLE', None)

		if sensor == "toolhead_filament_sensor":
			self.toolhead_filament_sensor.enable(enable == 1)

		if sensor == "feeder_sensor":
	...
	def calibrate_reverse_bowden_length(self):
		if self.toolhead_filament_sensor.filament_present:
			self.console_echo({
				'TITLE': "Calibrate bowden tube", 
				'MSG': 	"No calibration possible! Filament in hotend detected.", 
				'TYPE': "alert"
			})
			return
		...
		if len(self.parking_t_sensor_endstop) != self.tool_count:
			self.console_echo({
				'TITLE': "Calibrate bowden tube", 
				'MSG': 	"No calibration possible! Parking Tx endstops not available.", 
				'TYPE': "alert"
			})
			return
		...
		for i in range(0, self.tool_count):
			if not self.is_endstop_triggered(self.parking_t_sensor_endstop[i]):
				self.console_echo({
					'TITLE': "Calibrate bowden tube", 
					'MSG': 	"No calibration possible! Filament T" + str(i) + " is missing.", 
					'TYPE': "alert"
				})
				return
		...
		for i in range(0, self.tool_count):
			if self.is_endstop_triggered(self.parking_t_sensor_endstop[i]):
				...
				if self.is_endstop_triggered(self.parking_t_sensor_endstop[i]):
					self.console_echo({
						'TITLE': "Calibrate bowden tube", 
						'MSG': 	"Can not calibrate reverse bowden length bc the filament could not be loaded into its parking sensor!", 
						'TYPE': "alert"
					})
					self.select_filament(-1)
					return
				...
				for m in range(max_step_count):
					...
					if self.is_endstop_triggered(self.toolhead_sensor_endstop):
						...
						self.console_echo({
							'TITLE': "Calibrate bowden tube", 
							'MSG': 	"Reverse bowden length T" + str(i) + " = " + str(calibrated_reverse_bowden_length[i]) + "mm", 
							'TYPE': "success"
						})
						break
				...
				if not self.is_endstop_triggered(self.toolhead_sensor_endstop):
					self.console_echo({
						'TITLE': "Calibrate bowden tube", 
						'MSG': 	"Can not calibrate reverse bowden length bc the filament could not be loaded into the toolhead sensor!", 
						'TYPE': "alert"
					})
					self.select_filament(-1)
					return
		...
		if len(calibrated_reverse_bowden_length) == self.tool_count:
			...
			self.console_echo({
				'TITLE': "Calibrate bowden tube", 
				'MSG': 	"Average reverse bowden length = " + str(result) + "mm\nSetting has been saved and activated!", 
				'TYPE': "success"
			})
