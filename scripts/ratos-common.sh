...
ensure_sudo_command_whitelisting()
{
	...
	then
		rm /etc/sudoers.d/030-ratos-githooks
	fi
...
#EOF
	...
	chown root:root /tmp/030-ratos-githooks
	chmod 440 /tmp/030-ratos-githooks
	cp --preserve=mode /tmp/030-ratos-githooks /etc/sudoers.d/030-ratos-githooks
...
#EOF
		...
		chown root:root /tmp/031-ratos-change-hostname
		chmod 440 /tmp/031-ratos-change-hostname
		cp --preserve=mode /tmp/031-ratos-change-hostname /etc/sudoers.d/031-ratos-change-hostname
	fi
}
